...
m=5; %复制个数
load("highinmaxbutlowinaverage.mat");%导入初始种群
path=highinmaxbutlowinaverage;
%%临时变量
temp_path=path;
temp_bk=bk;
...
for i=1:episode
    ...
    %%评估
    for t=1:n
        grades(t,1)=assess(path(t,:),bk(t));%评估种群内的个体
    end
    e_best(i)=min(grades);
    temp_grades=grades;
    ...
    for t=1:n-m-1
        [temp_path(m+t,:),temp_bk(m+t),temp_path(m+t+1,:),temp_bk(m+t+1)]=cross(temp_path1(t,:),temp_path1(t+1,:),temp_bk1(t),temp_bk1(t+1));
    end
...
length=length(1);
disp("最终结果")
disp([5,path(c,1:bk(c)),5]);
disp([5,path(c,bk(c)+1:length),5]);
disp(["时间：",(assess(path(c,:),bk(c)))/1.5]);