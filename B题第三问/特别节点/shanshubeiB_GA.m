...
for i=1:episode
    ...
    %%评估
    for t=1:n
        grades(t,1)=assess(18,path(t,:),bk(t));%评估种群内的个体
    end
    e_best(i)=min(grades);
    temp_grades=grades;
    ...
    for t=1:n-m-1
        [temp_path(m+t,:),temp_bk(m+t),temp_path(m+t+1,:),temp_bk(m+t+1)] = ...
            cross_speci(temp_path1(t,:),temp_path1(t+1,:),temp_bk1(t),temp_bk1(t+1));
    end
...
x=1:episode;
plot(x,e_best);
saveas(gcf,num2str(18),'png');
c=find(temp_grades==min(temp_grades));
c=c(1);%找到最优方案的索引
length=size(nonzeros(path(c,:)));
length=length(1);
disp(["最终结果(起始节点为",18,")"]);
disp([18,path(c,1:bk(c)),18]);
disp([18,path(c,bk(c)+1:length),18]);
disp(["时间：",(assess(18,path(c,:),bk(c)))/1.5]);
